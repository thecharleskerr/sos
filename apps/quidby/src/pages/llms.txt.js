...
export async function GET() {
  ...
  const phoneList = Object.values(phones).map((p) => ({ slug: p.slug, name: p.name }));
  return new Response(llmsTxt(getSite(), posts, { networks: nets, categories: cats, compares, phones: phoneList }), { headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
}
