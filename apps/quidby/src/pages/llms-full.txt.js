...
export async function GET() {
  ...
  const phoneRows = Object.values(phones).map((p) => `| ${p.name} | ${p.maker} | ${p.rrpGBP !== null ? `${gbp(p.rrpGBP)}${p.storageGB ? ` for ${p.storageGB}GB` : ''}` : 'not stated'} | ${p.updates ?? 'not stated'} | ${p.checked} |`);
  const phoneTable = ['| Phone | Maker | Maker\'s UK price | Software updates | Checked |', '|---|---|---|---|---|', ...phoneRows].join('\n');
  return new Response(llmsFullTxt(getSite(), posts, [{ title: 'Every network, from its own pages', body: table }, { title: 'Every phone we track, from the maker\'s own pages', body: phoneTable }]), { headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
}
