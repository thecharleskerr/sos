...
export async function GET() {
  ...
  const extra = [
    ...categoryKeys.map((k) => ({ path: `/sims/${k}/`, changefreq: 'weekly', priority: 0.8 })),
    ...Object.keys(phones).map((k) => ({ path: `/phones/${k}/`, changefreq: 'weekly', priority: 0.8 })),
  ...
  return new Response(sitemapXml(getSite(), posts, { today, extra }), { headers: { 'Content-Type': 'application/xml; charset=utf-8' } });
}
