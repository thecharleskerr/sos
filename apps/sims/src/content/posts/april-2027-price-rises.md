---
title: "April 2027 price rises: what each UK network will add to your bill"
description: "A network by network table of confirmed April 2027 mobile price rises in pounds and pence, taken from each network's own terms and dated."
publishDate: 2026-11-30
checked: 2026-09-04
query: "April 2027 price rises"
answer: "O2 and Vodafone state a fixed £2.50 a month rise each April, Three's rise is £1.80 to £2.30 depending on plan size, and EE's £2.50 rise lands on 31 March, not April. SMARTY, giffgaff, iD Mobile, Lebara, Talkmobile, Simp and Ecotalk state no mid-contract price rise at all."
deals: ["deal-of-week", "cheapest"]
cta: { label: "See no price rise deals", href: "/deals/no-price-rise/" }
sources:
  - { name: "O2, Prices", url: "https://www.o2.co.uk/prices", checked: 2026-09-02 }
  - { name: "Vodafone, Annual price changes", url: "https://www.vodafone.co.uk/pricechanges", checked: 2026-09-02 }
  - { name: "Three, Changes to your monthly charge, customers joining or upgrading on or after 9 November 2025", url: "https://www.three.co.uk/support/bills-and-contracts/your-monthly-plan-and-usage/price-increase-9nov25", checked: 2026-09-02 }
  - { name: "EE, SIM only plan terms and conditions, from 7 August 2025", url: "https://ee.co.uk/content/dam/help/terms-and-conditions/price-plans/mobile/pay-monthly-price-plans/ee-simo-plan-tncs-from-7-august-2025.pdf", checked: 2026-09-04 }
  - { name: "SMARTY, Cut your mobile costs and save money with SMARTY", url: "https://smarty.co.uk/blog/save-money-with-smarty", checked: 2026-09-02 }
  - { name: "giffgaff, Why giffgaff", url: "https://www.giffgaff.com/why-giffgaff", checked: 2026-09-02 }
  - { name: "iD Mobile, Price increases", url: "https://www.idmobile.co.uk/price-increases", checked: 2026-09-02 }
  - { name: "Lebara, No price rise disclaimer", url: "https://www.lebara.co.uk/en/no-price-rise-disclaimer.html", checked: 2026-09-02 }
  - { name: "Talkmobile, Price rises", url: "https://talkmobile.co.uk/price-rises", checked: 2026-09-04 }
  - { name: "Simp, Students", url: "https://simpmobile.com/students", checked: 2026-09-02 }
  - { name: "Ecotalk, Terms and conditions", url: "https://www.ecotalk.co.uk/terms-and-conditions", checked: 2026-09-04 }
  - { name: "EE, Guide to your bill: about annual price changes", url: "https://ee.co.uk/help/billing-payments/guide-to-bill/about-annual-prices-changes", checked: 2026-09-04 }
  - { name: "Ofcom, Ofcom bans mid-contract price rises linked to inflation", url: "https://www.ofcom.org.uk/phones-and-broadband/bills-and-charges/ofcom-bans-mid-contract-price-rises-linked-to-inflation", checked: 2026-09-02 }
  - { name: "Ofcom, Switching mobile phone provider", url: "https://www.ofcom.org.uk/phones-and-broadband/switching-provider/switching-mobile-phone-provider", checked: 2026-09-02 }
  - { name: "Ofcom, General Condition C7, switching", url: "https://www.ofcom.org.uk/siteassets/resources/documents/phones-telecoms-and-internet/information-for-industry/general-authorisation-regime/unofficial-consolidated-general-condition-extract-c7---switching", checked: 2026-09-02 }
faq:
  - { q: "Will UK mobile networks raise prices in April 2027?", a: "Several have said so already. O2 and Vodafone state a £2.50 a month rise each April, Three's rise is £1.80 to £2.30 depending on plan size, and EE's £2.50 rise lands a day earlier, on 31 March. SMARTY, giffgaff, iD Mobile, Lebara, Talkmobile, Simp and Ecotalk state no mid-contract rise at all." }
  - { q: "Why does EE's price rise land on 31 March and not April?", a: "EE's own terms, in force from 7 August 2025, state the monthly plan price for pay monthly and SIM only plans increases by £2.50 on 31 March each year, a day ahead of the 1 April date other networks use. EE is shown here for its published policy only, since this site does not carry EE as an affiliate deal." }
  - { q: "How much extra will Three's price rise add to my bill?", a: "It depends on your plan's data allowance. Three states £1.80 a month on plans of 4GB or less, £1.90 a month on plans from 5GB to 99GB, and £2.30 a month on plans of 100GB or more, for customers who joined or upgraded on or after 9 November 2025." }
  - { q: "Which UK networks say they will not raise prices?", a: "SMARTY, giffgaff, iD Mobile, Lebara, Talkmobile, Simp and Ecotalk each state no mid-contract price rise on their own pages. iD Mobile's promise covers SIM only plans specifically, with its fixed rise applying only to handset plans instead." }
  - { q: "How do I switch before my network's April 2027 rise lands?", a: "Text PAC to 65075 to request a code that keeps your number, which Ofcom says is valid for 30 days and should normally have you ported and ready with a new provider within one working day, with no charge for a notice period running after the switch date." }
draft: false
---

Knowing your April 2027 price rise now, not after the first bill lands, is what lets you leave before it costs you anything. Get this wrong and you pay the rise for every month left on your contract, on top of the price you signed up for.

The figures below come from each network's own pricing and terms pages, and each one carries the date we checked it. We recheck the deal card above every Monday, so the price and the rise it states stay current even though this table does not change week to week.

## What each network says it will add in April 2027

Several UK networks state a mid-contract rise as a fixed amount in pounds and pence that lands on a set date each year, a way of stating it that Ofcom has required at the point of sale since January 2025. The table below is what each network's own pages state now for the plans it sells today. A row marked not stated is one where we found nothing on the network's own site confirming a rise or ruling one out. That is not the same as no price rise, so do not read it as good news.

| Network | Monthly rise | When it lands | Applies to | Checked |
|---|---|---|---|---|
| O2 | £2.50 (75p on data-only and smartwatch plans) | April | Airtime plans; device cost frozen | 2 September 2026 |
| Vodafone | £2.50 | 1 April | Pay monthly airtime and data plans; Device Plans unaffected | 2 September 2026 |
| Three | £1.80 to £2.30, by plan size (see tiers below) | 1 April | Customers joining or upgrading from 9 November 2025; device payments excluded | 2 September 2026 |
| EE | £2.50 | 31 March | Pay monthly and SIM only plans under terms from 7 August 2025; not a deal on this site | 4 September 2026 |
| SMARTY | No price rise | Not applicable | All plans | 2 September 2026 |
| VOXI | Not stated | Not stated | Not stated | Not stated |
| giffgaff | No price rise | Not applicable | 18 month contracts | 2 September 2026 |
| iD Mobile | No price rise | Not applicable | SIM only plans | 2 September 2026 |
| Tesco Mobile | Not stated | Not stated | Not stated | Not stated |
| Lebara | No price rise | Not applicable | All plans | 2 September 2026 |
| Talkmobile | No price rise | Not applicable | All plans | 4 September 2026 |
| Sky Mobile | Not stated | Not stated | Not stated | Not stated |
| BT Mobile | Not stated | Not stated | Not stated | Not stated |
| Asda Mobile | Not stated | Not stated | Not stated | Not stated |
| 1pMobile | Not stated | Not stated | Not stated | Not stated |
| spusu | Not stated | Not stated | Not stated | Not stated |
| Lycamobile | Not stated | Not stated | Not stated | Not stated |
| Mozillion | Not stated | Not stated | Not stated | Not stated |
| Simp | No price rise | Not applicable | All plans | 2 September 2026 |
| Honest Mobile | Not stated | Not stated | Not stated | Not stated |
| Revolut Mobile | Not stated | Not stated | Not stated | Not stated |
| Klarna Mobile | Not stated | Not stated | Not stated | Not stated |
| Utility Warehouse | Not stated | Not stated | Not stated | Not stated |
| Your Co-op Mobile | Not stated | Not stated | Not stated | Not stated |
| Ecotalk | No price rise | Not applicable | All plans | 4 September 2026 |

Three's rise depends on plan size, so there is no single figure. For customers joining or upgrading on or after 9 November 2025, Three's support page sets out three tiers:

1. Plans of 4GB or less, and Smartwatch Pairing Plans: £1.80 a month.
2. Plans from 5GB to 99GB: £1.90 a month.
3. Plans of 100GB or more: £2.30 a month.

Device payments are excluded from every tier.

EE's rise lands on 31 March, a day ahead of the 1 April date Vodafone and Three state. EE's own terms, in force from 7 August 2025, state the monthly price for pay monthly and SIM only plans increases by £2.50 on 31 March each year. The rise does not apply if the plan is sold as a fixed price plan. EE is shown here for its published policy only. This site never carries EE deals in a comparison table, so there is nothing to click through to on EE above.

Seven networks state the opposite. SMARTY says its plans are not linked to CPI inflation and will not increase each April. giffgaff says there are no mid-contract price rises on its 18 month contracts. Lebara says it has not raised its prices since 2020. Talkmobile and Ecotalk each carry a page stating no annual price rises outright. iD Mobile's no-rise promise covers SIM only plans, with its fixed rise landing only on handset plans instead. Simp states there is no contract and no mid-contract price rise.

## Here is exactly what to do

1. Open the deal card above and read its price rise line: a figure in pounds and pence, or "No price rise", rechecked every Monday.
2. Find your network in the table above and note its monthly rise and the date it lands.
3. If your network's row says not stated, treat that as unresolved, not safe. Check your own contract for the figure rather than assuming there is none.
4. If your network states a fixed rise, work out what it adds using the worked example below, then compare that total against a no price rise network's plan.
5. If you want to move, request a PAC or STAC now. Ofcom's switching rules mean the switch does not have to wait until your rise actually lands.
6. [See no price rise deals](/deals/no-price-rise/) and pick a plan before the date in step two arrives.

## Will the April 2027 rise apply to my own contract?

It depends on when you took the plan out, not just which network you are on. Two of the fixed rises above are cohort specific. Three's £1.80 to £2.30 tiers apply to customers joining or upgrading on or after 9 November 2025. EE's £2.50 figure applies to plans taken under the terms in force from 7 August 2025, unless sold as a fixed price plan. If you signed up earlier, the figure that applies to you may differ from the table, and only your own network can confirm which terms your plan sits under.

O2 and Vodafone state their rises without naming a cohort start date, so their £2.50 figures apply more broadly, again with the device part of a phone contract frozen. If your contract includes a phone rather than being SIM only, [Save on Smartphones](https://saveonsmartphones.co.uk/) tracks the device side separately.

## How much would a rise like this actually cost you?

Multiply the monthly rise by the months left on your contract, then add that to the total contract cost the card above already shows. EE's own guide to your bill gives a worked example worth borrowing, even though EE deals never appear as a clickable pick on this site. It states that a SIM only plan taken out at £25 a month on a 24 month contract rises to £27.50 a month from 31 March 2026, then to £30 a month from 31 March 2027. That is not a price this site sells. It simply shows how a fixed monthly rise, however small it looks on the card, adds up once you count every month left on the contract.

This is why the deal card above states the total contract cost and the price rise as separate lines, rather than folding the rise into one monthly figure.

## How do you leave before the rise lands?

You do not have to wait for a rise to hit your bill before you switch. Ofcom's guidance on switching mobile provider says you can request a PAC by texting PAC to 65075 to keep your number, and once you have a PAC it is valid for 30 days. Your current provider must reply within one minute of the text, and once you have handed the PAC over, your number should normally be ported and ready within one working day. Ofcom's rules also mean your old provider must not charge you for a notice period that runs after the switch date. [Our full switching guide](/blog/how-to-switch-mobile-network-and-keep-your-number/) sets out the rest of the process, including a STAC if you do not want to keep your number.

Switching before the rise date matters more than switching after it. Once the new price is in force, cancelling still means paying it until your new PAC completes.

## What to do now

Read the price rise line on the card above first, then check your own network against the table. If your row shows a fixed rise, or is not stated, the table above and [our companion guide on how these rises work](/blog/mid-contract-price-rises-explained/) are the reference to keep. [See no price rise deals](/deals/no-price-rise/) and pick a plan the site rechecks every Monday, before April 2027 arrives. The same figures sit on the [homepage](/), and the full [network reference](/networks/) lists which host network each one runs on.
