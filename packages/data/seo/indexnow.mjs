/* Submits every URL in the built sitemap to IndexNow, which Bing, Yandex,
   Naver and Seznam share. Google does not use it. The key is public by
   design: the protocol proves ownership by serving <key>.txt from the site
   root, so the key file lives in the app's public/ directory.

     node packages/data/seo/indexnow.mjs
     node packages/data/seo/indexnow.mjs --dry-run */
...
const pub = resolve(root, 'apps/quidby/public');
...
const sitemap = resolve(root, 'apps/quidby/dist/sitemap.xml');
